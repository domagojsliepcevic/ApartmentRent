...
namespace ApartmentRent.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> AddToOwner(string id)
        {
            IdentityRole ownerRole = await roleManager.FindByNameAsync("ApartmentOwner");
            if (ownerRole == null)
            {
                TempData["message"] = "Owner role does not exist. "
                    + "Click 'Create Owner Role' button to create it.";
            }
            else
            {
                User user = await userManager.FindByIdAsync(id);
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveFromOwner(string id)
        {
            User user = await userManager.FindByIdAsync(id);
            var result = await userManager.RemoveFromRoleAsync(user, "ApartmentOwner");
            if (result.Succeeded) { }
            return RedirectToAction("Index");
        }
